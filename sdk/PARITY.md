...
| Webhooks: `DELETE /v1/webhooks/{id}?tenantId=` unregister | ✅ `client.webhooks.unregister(subscriptionId, tenantId?)` (SDK-3, 2026-05-15; `tenantId` 2026-09-26, #50 — omitting it is deprecated) | ✅ `client.webhooks_unregister(subscription_id, tenant_id=None)` (SDK-3, 2026-05-15; `tenant_id` 2026-09-26, #50 — omitting it warns `DeprecationWarning`) | ✅ `client.UnregisterWebhookForTenant(ctx, subscriptionID, tenantID)` (2026-09-26, #50); `UnregisterWebhook(ctx, subscriptionID)` kept, `Deprecated:` (sends no `tenantId`) |
