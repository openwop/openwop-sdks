...
describe('content + trigger REST helpers', () => {
  it('getPage maps to GET /v1/content/pages/{slug} + forwards Accept-Language', async () => {
    ...
    expect(captured[0]?.url).toContain('/v1/content/pages/home');
  ...
  it('putSection maps to PUT with the locale+data body', async () => {
    ...
    expect(captured[0]?.url).toContain('/v1/content/pages/p1/sections/s1');
    expect(JSON.parse(captured[0]!.body!)).toEqual({ locale: 'fr-FR', data: { title: 'Bonjour' } });
  });

  it('triggerSubscriptions.create maps to POST /v1/trigger-subscriptions', async () => {
    const { client, captured } = mockClient(() => ({
      status: 201,
