...
describe('browser safety of the package barrel', () => {
  ...
  it('no barrel-reachable module imports a Node builtin — 2.0.0 dropped the deprecated webhook re-export', () => {
    // The guard that matters over time: this catches the NEXT module that
    // reaches for node:*, which is how the 1.x barrel broke in the first
    // place. In 2.x the verifiers live only on the `./webhooks` subpath.
    ...
    expect(readFileSync(join(ROOT, 'src/index.ts'), 'utf8')).not.toMatch(/from '\.\/webhook-helpers\.js'/);
  });
});
